...
function main() {
  ...
  while (1) {
    switch (command) {
      case 1:
        let studentInfo = question('请输入学生信息（格式：姓名, 学号, 学科: 成绩, ...），按回车提交：').trim();
        while (getStudent(studentInfo) === -1) {
          studentInfo = question('请按正确的格式输入（格式：姓名, 学号, 学科: 成绩, ...）：').trim();
        }
        ...
        break;
      case 2:
        let studentsId = question('请输入要打印的学生的学号（格式： 学号, 学号,...），按回车提交：').trim();
        while (printScoreSheet(students, studentsId) === -1) {
          studentsId = question('请按正确的格式输入要打印的学生的学号（格式： 学号, 学号,...），按回车提交：').trim();
        }
